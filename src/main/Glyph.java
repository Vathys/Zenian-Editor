...
public class Glyph implements Cloneable
{
     ...
     private int[] offset; // {x, y, xOff, yOff}

     public Glyph(GlyphColumn gc)
     {
          this.fontSize = gc.getFontSize();
          this.fontColor = gc.getFontColor();
          this.x = gc.getStartX();
     ...
     private int[] pickRender(Graphics2D gi, LetterRenderHandler rh, int[] offset, ZenianLetter letter)
     {
          ...
          //System.out.println("Offset for " + letter.name() + "(" + offset[2] + ", " + offset[3] + ")");
          ...
          switch (letter)
          {
               ...
               case J:
                    ...
                    break;
               case TURNED_R:
                    off = rh.renderU0279(gi, offset[0], offset[1], offset[2], offset[3]);
                    break;
               case THETA:
     ...
     public ArrayList<ZenianLetter> getLetters()
     {
          return c;
     }
     ...
     public String writeGlyph()
     {
          String id = "{";

          if(c.size() > 0)
          {
               id += c.get(0).getUnicodeCharacter();
          }
          
          for (int i = 1; i < c.size(); i++)
          {
               id += ", " + c.get(i).getUnicodeCharacter();
          }
     ...
     {
          Glyph clone = new Glyph(this.gc);
          for (ZenianLetter l : c)
          {
               clone.addLetter(l);
          }
          return clone;
